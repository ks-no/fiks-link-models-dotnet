pipeline {
    ...
    parameters {
        ...
        string(name: "triggerbranch", defaultValue: "main", description: "Hvilke branch fra KS.Fiks.Link.Specification skal det bygges fra?" )
    }
    environment {
        MODELS_FOLDER = 'KS.Fiks.Link.Models.V1'
        GENERATOR_FOLDER = 'KS.Fiks.Link.JsonModelGenerator'
    }
    stages {
        stage('Initialize') {
            steps {
        ...
        stage('Fetch specification') {
          steps { 
            dir('temp') {
              ...
              url: 'https://github.com/ks-no/fiks-link-specification.git'
